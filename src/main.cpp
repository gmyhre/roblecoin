...
#if defined(NDEBUG)
# error "Roblecoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Roblecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Roblecoin Signed Message:\n";
...
{
    // Roblecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Roblecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Roblecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Roblecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Roblecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Roblecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("046d639f7906cfaf66dcadeff6625fdd476cf929c4d84326d32f662c2910730d552bc4c4509a63d43cea901986355398f68e1d371fc282e7a838604c638ee56497") << OP_CHECKSIG;
        CBlock block;
...
unsigned char pchMessageStart[4] = { 0xf4, 0xc4, 0xb4, 0xd4 }; // Roblecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RoblecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RoblecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RoblecoinMiner(CWallet *pwallet)
{
    printf("RoblecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("roblecoin-miner");
    ...
    try { loop {
        ...
        printf("Running RoblecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RoblecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RoblecoinMiner, pwallet));
}
